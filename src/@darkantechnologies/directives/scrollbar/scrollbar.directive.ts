...
import { ScrollbarGeometry, ScrollbarPosition } from '@darkantechnologies/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[darkantechnologiesScrollbar]',
    exportAs: 'darkantechnologiesScrollbar'
})
export class DarkanTechnologiesScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_darkantechnologiesScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() darkantechnologiesScrollbar: boolean = true;
    @Input() darkantechnologiesScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'darkantechnologiesScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.darkantechnologiesScrollbar = coerceBooleanProperty(changes.darkantechnologiesScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.darkantechnologiesScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'darkantechnologiesScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.darkantechnologiesScrollbarOptions.currentValue);
    ...
    isEnabled(): boolean
    {
        return this.darkantechnologiesScrollbar;
    }
    ...
    private _init(): void
    {
        ...
        {
            this.darkantechnologiesScrollbar = false;
            return;
        }
