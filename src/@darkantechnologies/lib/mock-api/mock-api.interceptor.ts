...
import { DarkanTechnologiesMockApiService } from '@darkantechnologies/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root'
})
export class DarkanTechnologiesMockApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(DARKAN_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _darkantechnologiesMockApiService: DarkanTechnologiesMockApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._darkantechnologiesMockApiService.findHandler(request.method.toUpperCase(), request.url);
