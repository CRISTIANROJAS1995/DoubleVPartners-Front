...
// -----------------------------------------------------------------------------------------------------
// @ Double V Partners TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--darkantechnologies-${e(key)}`, value], [`--darkantechnologies-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            ]))
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`darkantechnologies.customProps.background.${colorScheme}`);
            const foreground = theme(`darkantechnologies.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--darkantechnologies-${e(key)}`, value], [`--darkantechnologies-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--darkantechnologies-${e(key)}`, value], [`--darkantechnologies-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--darkantechnologies-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--darkantechnologies-on-${name}-rgb), <alpha-value>)`]
                    ])))
                },
                darkantechnologies  : {
                    customProps: {
