...
import { DoublevPartnersMockApiService } from '@doublevpartners/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root'
})
export class DoublevPartnersMockApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(DOUBLEVPARTNERS_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _doublevpartnersMockApiService: DoublevPartnersMockApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._doublevpartnersMockApiService.findHandler(request.method.toUpperCase(), request.url);
