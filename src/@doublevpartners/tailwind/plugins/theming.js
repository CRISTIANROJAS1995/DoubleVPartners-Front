    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--doublevpartners-${e(key)}`, value], [`--doublevpartners-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
            ]))
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`doublevpartners.customProps.background.${colorScheme}`);
            const foreground = theme(`doublevpartners.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--doublevpartners-${e(key)}`, value], [`--doublevpartners-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--doublevpartners-${e(key)}`, value], [`--doublevpartners-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--doublevpartners-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--doublevpartners-on-${name}-rgb), <alpha-value>)`]
                    ])))
                },
                doublevpartners  : {
                    customProps: {
